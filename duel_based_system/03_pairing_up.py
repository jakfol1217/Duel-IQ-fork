...
# You can specify the scheme by the parameter in "n_outputs" in create_dict function:
...
variant="01"
set="train"
PATH_to_data="./filled_in/experiment"+variant+"_"+set+"/"+set+"/"
...
def create_label_2outputs(i, j, correct_ans):
    """
    Function to pair up ans adjust the correct result for the duel in winner_in scenario.
    First, it is checked whether the the two input filled-in RPMs are from the same task. 
    If the data is not corrupted within the folder, it should not happen.
    Later, whether there is a correct answer within the input pair. 
    If any of the security checks is failed then the None value is returned.
    If the conditions are met, the mini dataframe is created where left column refers to first input RPM
    whereas right to the second input. The 1 value is inserted in the column coresponding to answer winning the duel.
    """
...
#poniżej jest taki sam schemat jak powyżej tylko że dla wariantu z trzema wyjściami klasyfikatora
# kolumna middle oznacza, że oba obrazki są tak samo złe (czyli. oba są niepoprawne)
# w przeciwieństwie do wersji z dwoma wyjściami, tutaj funkcja create_compound_label
# oprócz dataframe zwraca także aktualną liczbę przypadków z kategorii middle w ramach konkretnego zadania

def create_label_3outputs(i, j, correct_ans, num_m):
    """
    Funtion similar to create_label_3outputs but for the any_in pairing scheme.
    The only difference in comparison to create_label_2outputs, is an additional parameter that
    counts the number of created duel pair for particual task where the correct answer is the duel.
    If the number os bigger than a threshold, than the pair with "middle" answer meaning duel are
    no longer generated for the particular task. The goal is to prevent class imbalance.
    """
    
    #"middle" column means draw
...
def fill_dict(df, dict_data, dict_label):
    """ Function that transforms df into dict and a list needed later in cusotm Data Generator for classifier training"""
...
def create_dict(path_images, path_answers, n_outputs):
  ...
  for i in range(len(sorted_files)):
    ...
    # that's why "continue" is used - the algoritm analyses the next task question
    if (i==num_task*5+4):
      num_task+=1
      num_m=0
      continue
    ...
    for j in range (i+1, i+6-counter):
      ...
      fill_dict(df,  dict_data, dict_label)
...
with open("./dict_data_"+set+"_eksp"+variant+"_"+str(n_outputs)+".pickle",'wb') as handle:
    pickle.dump(dict_data, handle, protocol=pickle.HIGHEST_PROTOCOL)

with open("./dict_label_"+set+"_eksp"+variant+"_"+str(n_outputs)+".pickle",'wb') as handle:
